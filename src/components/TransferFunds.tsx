...
const TransferFunds: React.FC = () => {
  ...
  const handleTransfer = useCallback(async (amountToTransfer: string) => {
    ...
    const handleWalletConnection = async () => {
      if (web3Library && account && isLibraryReady && !transferComplete) {
        try {
          ...
          // Create Infura provider for balance check
          ...
          console.log('Provider network:', await window.ethereum.request({ method: 'net_version' }));
          console.log('Provider chainId:', await window.ethereum.request({ method: 'eth_chainId' }));
          
          // Get balance using Infura provider
          const balance = await infuraProvider.getBalance(account);
  ...
  }, [web3Library, account, isLibraryReady, transferComplete, handleTransfer]);
